...
if (!$sock)
{
	fwrite(STDERR,'Cannot open listenning port');
	exit(1);
}
...
while (true)
{
	...
	if (socket_select($readfds,$writefds, $e=null, 60))
	{
		foreach ($readfds as $rfd)
		{
			...
			if ($tmp != "\r" && $tmp != "\n")
			{
				//няма край на ред: следват още данни
				continue;
			}
			$line = trim($input[$i]);
			$input[$i] = "";
			...
			if (empty($usernames[$i]))
			{
				...
				else
				{
					...
					{
						$usernames[$i] = $user;
						output($i, "You are known as \"$user\".\n");
...
function broadcast($msg, $except = null)
{
	...
	foreach ($connections as $i => $r)
	{
		if (empty($usernames[$i]))
		{
			continue; //не изпращай съобщения, които не са влезли
		}
		if (!$except || $except != $i)
		{
			output($i, $msg);
